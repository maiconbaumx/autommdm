...
COUNTER=$(df -h | awk '{print $6}' | awk '{if (NR!=1) {print}}' | wc -l)
         ...
         do
             ~/autommdm/aws-scripts-mon/mon-put-instance-data.pl --disk-path=`df -h | awk '{print $6}' | grep -n ^ | cut -d: -f2 | awk '{if (NR!=1) {print}}' | awk NR==$i`  --disk-space-used --disk-space-avail --disk-space-util #--aws-credential-file=~/autommdm/aws-scripts-mon/awscreds.template
         done
